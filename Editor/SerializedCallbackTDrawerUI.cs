...
namespace Rails.Editor.PropertyDrawers
{
	[CustomPropertyDrawer(typeof(SerializableCallback<>), true)]
	public class SerializedCallbackTDrawerUI : PropertyDrawer
	{
		...
		private void ShowMethodDropdown(Object target, SerializedProperty methodProp, SerializedProperty property, Button methodButton, VisualElement root)
		{
			...
			{
				List<MethodInfo> methods = new(FindTargetMethods(targetType));
				if (target is GameObject gameObject)
				{
					var components = gameObject.GetComponents<Component>();
					methods.AddRange(components.SelectMany(x => FindTargetMethods(x.GetType())));
				}
			...
			IEnumerable<MethodInfo> FindTargetMethods(Type targetType)
			{
